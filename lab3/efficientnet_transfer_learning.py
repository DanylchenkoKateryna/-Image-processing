"""
Лабораторна робота №3 — Секція 1.3
EfficientNetB0: Transfer Learning + Fine-tuning + Оцінка якості на CIFAR-10
"""
...
# ══════════════════════════════════════════════════════════════════
# 1.3.1  Transfer Learning — заморожена база
# ══════════════════════════════════════════════════════════════════
print("\n=== 1.3.1  Transfer Learning EfficientNetB0 (frozen) ===")
...
cb = [
    keras.callbacks.ModelCheckpoint("effnet_b0_tl.keras",
...
# ══════════════════════════════════════════════════════════════════
# 1.3.2  Fine-tuning — верхні 40 шарів
# ══════════════════════════════════════════════════════════════════
print("\n=== 1.3.2  Fine-tuning (top 40 layers) ===")
base.trainable = True
...
cbs_ft = [
    keras.callbacks.ModelCheckpoint("effnet_b0_ft.keras",
                                    save_best_only=True, monitor="val_accuracy"),
    keras.callbacks.ReduceLROnPlateau(monitor="val_loss", factor=0.5, patience=2, min_lr=1e-6),
    keras.callbacks.EarlyStopping(patience=5, restore_best_weights=True, monitor="val_accuracy"),
]
model.fit(train_ds, validation_data=val_ds, epochs=1, callbacks=cbs_ft, verbose=1)

_, ft_acc = model.evaluate(test_ds, verbose=0)
print(f"Test acc (fine-tune): {ft_acc:.4f}")

# ══════════════════════════════════════════════════════════════════
# 1.3.3  Оцінка та помилки
# ══════════════════════════════════════════════════════════════════
print("\n=== 1.3.3  Оцінка (precision / recall / F1) ===")
...
plt.title("Матриця плутанини — EfficientNetB0"); plt.tight_layout()
plt.savefig("effnet_confusion_matrix.png", dpi=100); plt.show()
...
if k:
    ...
    for i, idx in enumerate(err_idx[:k], 1):
        ...
        plt.axis("off")
    plt.suptitle(f"Помилки: {CLASS_NAMES[cls_t]}↔{CLASS_NAMES[cls_p]} (EfficientNetB0)")
    plt.tight_layout(rect=[0,0,1,0.93])
    plt.savefig("effnet_error_examples.png", dpi=100); plt.show()

print(f"\nГотово! TL={tl_acc:.4f} | Fine-tune={ft_acc:.4f}")
print("Файли: effnet_b0_tl.keras, effnet_b0_ft.keras")
